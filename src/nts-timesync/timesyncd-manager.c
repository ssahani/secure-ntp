...
static int manager_ntske_listen_setup(Manager *m) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to configure TLS manager: %m");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to connect to ntske server '%s'.", m->current_ntske_server_name->string);
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to send request to ntske server '%s'.", m->current_ntske_server_name->string);
...
static int manager_ntske_resolve_handler(sd_resolve_query *q, int ret, const struct addrinfo *ai, void *userdata) {
        Manager *m = userdata;
        int r;
...
int manager_connect(Manager *m) {
        ...
        if (m->ntske && !m->current_ntske_server_name->addresses) {
                ...
                r = sd_resolve_getaddrinfo(m->resolve, &m->resolve_query, m->current_ntske_server_name->string, "123", &hints, manager_ntske_resolve_handler, m);
                if (r < 0)
